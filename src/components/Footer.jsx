...
export default function Footer({ locale }) {
  ...
  const handleAppStore = (placement) => {
    trackAppStoreClick(placement, typeof window !== 'undefined' ? window.location.pathname : '/')
  }
    ...
    <footer style={{ position: 'relative', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="section container" style={{ textAlign: 'center' }}>
        ...
        <a
          ...
          onClick={() => handleAppStore('footer_cta')}
        >
      ...
      {/* Internal Linking to SEO Guides */}
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="container" style={{ padding: '32px 24px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '24px' }}>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: '24px', flexWrap: 'wrap' }}>
            {links.map(({ label, href, external, isAppStore }) => (
              <a
                key={label}
                href={href}
                ...
                onClick={isAppStore ? () => handleAppStore('footer_link') : undefined}
